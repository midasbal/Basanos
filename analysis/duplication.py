...
def compute_duplication_stats(data_dir, room="lobby", use_verify_cache=True):
    ...
    records are read, counted, or how many re-verify -- see that module's
    docstring for why a cache hit is provably identical to a fresh
    `verify_record` call. Pass False to force full verification, bypassing
    the cache entirely (used by the tests proving the cache is transparent).
