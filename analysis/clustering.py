...
def compute_clustering_stats(
    data_dir, room="lobby", cap=DEFAULT_CAP, min_shared=DEFAULT_MIN_SHARED, use_verify_cache=True
):
    ...
    this room's verify cache. No did:key string, and no key-to-cluster
    membership of any kind, appears anywhere in the returned structure --
    see the module docstring's hard invariant and `_cluster_sizes`'s
    docstring for how that is enforced structurally, not just by
    convention.
    """
    _validate_room(room)
