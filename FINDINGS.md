...
This began as a single window of about a day. Continued capture has since extended it: over more than two unbroken days the throughput floor still never falls to quiet, and the curve rises and falls more than once, with recurring peaks. That is enough to rule out a single anomalous quiet-or-busy stretch, but not yet to confirm a clean twenty-four-hour cycle, since the peaks do not line up at the same hour from one day to the next. What holds across the longer record is the floor: sustained activity around the clock, with no nightly collapse a human-scheduled population would show.
...
Widening the gap sharpens the result rather than softening it, which is what a genuinely single-use population should do: the longer you wait, the fewer come back. Run on a later, longer stretch of continuous capture with a sixteen-hour gap instead of six, of 54,045 keys first seen in the cohort window, only 2.7 percent returned in a return window sixteen hours on, and at least 97.3 percent did not, with that return window captured at 87.0 percent. So across most of a day the non-return rate does not fall, it climbs, from 95.1 percent at six hours to 97.3 percent at sixteen. A key that appears once and is still absent the better part of a day later, in a room that never goes quiet, is not idling between messages.
...
The same bound as before applies, moved one step out. Return is measured within a fixed return window, not for all time, so a key absent from it could in principle surface days later. But the pattern holds and strengthens as the gap grows, and a population that appears once and stays silent across most of a day, in a room that never goes quiet, is not behaving like a set of engaged participants idling between messages. There were tens of thousands of such keys in every window measured.
