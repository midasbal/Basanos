...
A window is a single continuous stretch of capture. Two windows are reported here. They are not a time series, and the difference between their numbers should not be read as change over time. The reasons are under Caveats.
...
The two windows are not a time series. Window A is a continuous segment from one collector run. Window B is a single stretch captured in the roughly two hours after the collector restarted, so its coverage counters had reset and it reflects a short, recent slice rather than a continuation of window A. The large gap between their duplication rates, 25.8 versus 45.0 percent, cannot be attributed to change over time. It could be window length, the time of day, load conditions, or the restart boundary. Until capture is continuous across a known span, the two are two snapshots, not two points on a line.
