...
Re-running over time. A runner reruns the longitudinal measurements on a fixed window shape and archives each round, so repeated runs accumulate into a real time series rather than a single snapshot. Re-running is cheap now: verification results are cached per record, so a rerun over the grown capture only re-verifies what is new, dropping from many minutes to about one, which is what removed the growing-file cost that would otherwise have made frequent re-running impractical. The record is accumulated by running the runner every day or two and committing the archived rounds; wiring it to a timer is a later convenience, not a blocker. The cohort and diurnal measurements in particular sharpen with repetition over time, and the archived series is what a trend view would eventually be built on.
